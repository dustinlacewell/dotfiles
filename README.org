...
Finally, our list of Declaration Plugins is joined with our list of Definition
Modules to create our full import list.

All of the modules will either declare options, handle options, or define
option values. Typically plugins declare and handle options, while definition modules
define their values.
...
relies on is technically an input to that cryptographic hash. A set is created
...
To specify it's content we assign a path pointing to our file. The file
exists in the Nix store, in the package built by the ~runCommand~ function we
called. It will exist again in the store, in our Home Manager built package
once the build is complete. But then the original version from the sub-build
